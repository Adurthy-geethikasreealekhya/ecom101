...
def admincreate():
    if request.method=='POST':
        ...
        print(request.form)#accessing data from frontend as immutabledictionary format by name attribute,function names should not repeat
        try:
            ...
            if email_count[0] == 0:
                ...
                subject1 = 'Ecommerce Verification Code'
                body1 = f'Ecommerce Otp for admin registration {otp}'
                sendmail(to = aemail,subject=subject1,body=body1)
                flash('Otp has sent to given mail')
                return redirect(url_for('aotp',paotp = encode(data = admindata)))#passing encrpted otp
            else:
                flash('wrong email')
                return redirect(url_for('admincreate'))
...
def adminforgot():
    if request.method=='POST':
        ...
        if stored_email[0] == 1:
            subject = 'Admin Password Reset link for ecommee aplication'
            ...
            flash('Reset link has sent given Email')
            return redirect(url_for('adminforgot'))
        elif stored_email[0] == 0:
            flash('No email registred please check')
            return redirect(url_for('adminforgot'))
...
def additem():
    if session.get('admin'):
        if request.method=='POST':
            ...
            filename = otpgen() + '.' + item_img.filename.split('.')[-1]#fetching extension
            drname = os.path.dirname(os.path.abspath(__file__))#D:\pfs6\ecom
            print(drname)
            static_path = os.path.join(drname,'static')#D:\pfs6\ecom\static
            print(static_path)
            item_img.save(os.path.join(static_path,filename))#the image will save on static
            try:
                cursor = mydb.cursor(buffered=True)
...
def delete_item(item_id):
    if session.get('admin'):
        try:
            ...
            stored_imgname = cursor.fetchone() #('FG5grt.png')
        ...
        try:
            ...
            if request.method == 'POST':
                ...
                if filename == '':#not uploading img
                    ...
                    img_name = otpgen()+'.'+filename.split('.')[-1]
                    drname = os.path.dirname(os.path.abspath(__file__))#D:\pfs6\ecom
    ...
    if session.get('admin'):
        try:
            ...
            if request.method == 'POST':
                ...
                item_img = request.files['file']
                filename = item_img.filename
                print(name,address,item_img)
                print(item_img)
                if item_img.filename == '':
                    img_name = stored_itemdata[2]
                else:
                    img_name = otpgen()+'.'+item_img.filename.split('.')[-1] #new filename
                    drname = os.path.dirname(os.path.abspath(__file__))#D:\pfs6\ecom
                    static_path = os.path.join(drname,'static')
                    if stored_itemdata[2] in os.listdir(static_path):
                        os.remove(os.path.join(static_path,stored_itemdata[2]))
                    item_img.save(os.path.join(static_path,img_name))
                ...
                return redirect(url_for('updateadmin',email = session.get('admin')))

        return render_template('adminupdate.html',stored_itemdata = stored_itemdata)
    else:
        return redirect(url_for('admin'))
    ...
    if request.method=='POST':
        ...
        try:
            ...
            if email_count[0] == 0:
                ...
                subject1 = 'Ecommerce Verification Code'
                body1 = f'Ecommerce Otp for admin registration {otp}'
                sendmail(to = uemail,subject=subject1,body=body1)
                flash('Otp has sent to given mail')
                return redirect(url_for('uotp',puotp = encode(data = userdata)))#passing encrpted otp
        ...
        if request.method=='POST':
            ...
            try:
                ...
                if email_count[0] == 1:
                    ...
                    stored_password=cursor.fetchone()
                    if stored_password[0].decode('utf-8')==password:
                        session['user']=uemail #crating session
...
def userforgot():
    if request.method=='POST':
        ...
        stored_email = cursor.fetchone()
        if stored_email[0] == 1:
            subject = 'User Password Reset link for ecommee aplication'
